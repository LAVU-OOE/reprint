// ====================================================================
// app.js – LAVU Label Studio v9 with external data + consensus system
// ====================================================================
...
async function loadExternalData() {
    try {
        ...
        if (Array.isArray(sortimentData) && sortimentData.length > 0) {
            ...
            throw new Error('Invalid sortiment data');
        }
    } catch (err) {
...
function p1() {
    ...
    const item = a2.find(i => i.artNr === currentArtNr);
    if (item) {
...
function d1(a13) {
    const t = i18n[currentLang] || { confirmDelete: 'Delete?' };
...
function s4() {
    ...
    const t = i18n[currentLang] || { alertErrorChange: 'Error' };
    if (index === "" || !c1 || !c3) {
...
function e2() {
    try {
        ...
        console.error("Export failed:", a28);
...
function s3() {
    ...
    const c2 = document.getElementById('c2').value.trim();
    const c3 = document.getElementById('c3').value.trim();
    const t = i18n[currentLang] || { alertFillForm: 'Please fill', alertDuplicate: 'Duplicate' };
    if (!c1 || !c3) {
...
function u1() {
    const a29 = g1();
    const f = formats[currentFormatKey] || { cols: 3, rows: 8 };
    ...
    const t = i18n[currentLang] || { printLayout: 'Print Layout' };
    ...
    document.getElementById('fmt-overlay').innerHTML = `
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="3" x2="9" y2="21"/><line x1="15" y1="3" x2="15" y2="21"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="3" y1="15" x2="21" y2="15"/></svg> ${f.name || 'Format'}`;
    r2('t1', a29);
    r2('mdl', a29);
    r2('d5', a29);
...
function s6() {
    ...
    s7();
    const t = i18n[currentLang] || { alertSaved: 'Saved' };
    alert(t.alertSaved);
}
function s7() {
    const data = g1();
...
function changeFormat(key) {
    ...
    const f = formats[key] || { cols: 3, rows: 8 };
    const maxLabels = f.cols * f.rows;
    const i2 = document.getElementById('i2');
    const i3 = document.getElementById('i3');
    const t = i18n[currentLang] || { lblCount: 'Count', lblStartPos: 'Start' };
...
function f2() {
    ...
    fetch(a4, { cache: "no-store" })
        ...
        .then(function (data) {
            if (Array.isArray(data) && data.length > 0) {
                const merged = data.map(item => {
                    let geb = item.geb || '';
                    if (!geb) {
                        const localItem = a2.find(i => i.artNr === item.artNr);
                        if (localItem && localItem.geb) geb = localItem.geb;
                        else if (localOverrides[item.artNr]) geb = localOverrides[item.artNr];
                    }
                    return { artNr: item.artNr, bez: item.bez, geb };
                });
                a2 = merged;
                localStorage.setItem('lavu_studio_sortiment_v8', JSON.stringify(a2));
                document.getElementById('n1').innerHTML = (a4 === "sortiment.json") ? t.netSuccessLocal || 'Local' : t.netSuccessRemote;
        ...
        .catch(function (e) {
            ...
            document.getElementById('n1').innerHTML = (a4 === "sortiment.json") ? t.netFallbackLocal || 'Fallback local' : t.netFallbackRemote;
...
function applyLanguage() {
    ...
    document.getElementById('txt-studio-v9').textContent = t.studioV9 || 'Label Studio';
    const printLayoutEl = document.getElementById('txt-print-layout');
    printLayoutEl.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 2px;"><path d="M6 9V2h12v7M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg> ${t.printLayout || 'Print Layout'}`;
    document.getElementById('txt-art-nr').textContent = t.artNr || 'Art.Nr.';
    document.getElementById('txt-bezeichnung').textContent = t.bezeichnung || 'Bezeichnung';
    const printBtn = document.getElementById('btn-print-now');
    printBtn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg> ${t.printNow || 'Print Now'}`;
    const optionsBtn = document.getElementById('btn-options');
    optionsBtn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg> ${t.options || 'Options'}`;
    document.getElementById('txt-modal1-title').textContent = t.modal1Title || 'Settings';
    document.getElementById('lbl-location').innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg> ${t.lblLocation || 'Location'}`;
    document.getElementById('lbl-format').innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="3" x2="9" y2="21"/><line x1="15" y1="3" x2="15" y2="21"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="3" y1="15" x2="21" y2="15"/></svg> ${t.lblFormat || 'Format'}`;
    document.getElementById('txt-tab-select').textContent = t.tabSelect || 'Service-Worker';
    document.getElementById('txt-tab-manage').textContent = t.tabManage || 'Manage Database';
    document.getElementById('lbl-url').innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg> ${t.lblUrl || 'Sortiment API'}`;
    document.getElementById('lbl-locations-url').innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg> ${t.lblLocationsUrl || 'Locations API'}`;
    document.getElementById('btn-update').textContent = t.btnUpdate || 'Update';
    document.getElementById('btn-update-locations').textContent = t.btnUpdateLocations || 'Update';
    document.getElementById('lbl-db-suffix').textContent = t.lblDbSuffix || 'Container / Suffix';
    document.getElementById('lbl-db-bez').textContent = t.lblDbBez || 'Description';
    document.getElementById('btn-add-new').textContent = t.btnAddNew || 'Add New';
    document.getElementById('btn-cancel').textContent = t.btnCancel || 'Cancel';
    document.getElementById('btn-download-json').textContent = t.btnDownloadJson || 'Download JSON';
    document.getElementById('lbl-current-entries').textContent = t.lblCurrentEntries || 'Current entries:';
    document.getElementById('btn-save-default').innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg> ${t.btnSaveDefault || 'Save Defaults'}`;
    document.getElementById('btn-done').textContent = t.btnDone || 'Done';
    document.getElementById('modalTitle').textContent = t.modal2Title || 'Interactive A4 Print Sheet';
    const modalPrintBtn = document.getElementById('btn-modal-print');
    modalPrintBtn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg> ${t.btnModalPrint || 'Print'}`;
    document.getElementById('btn-modal-close').textContent = t.btnModalClose || 'Close';
    document.getElementById('txt-pwa-title').textContent = t.txtPwaTitle || 'Install as App';
    document.getElementById('txt-pwa-sub').textContent = t.txtPwaSub || 'Faster access & offline usage';
    document.getElementById('b3').textContent = t.btnPwaInstall || 'Install';
    document.getElementById('layout-title-attr').setAttribute('title', t.layoutTitleAttr || 'Click for full preview');
    const f = formats[currentFormatKey] || { cols: 3, rows: 8 };
    const maxLabels = f.cols * f.rows;
    document.getElementById('lblCount').textContent = `${t.lblCount || 'Count'} (max. ${maxLabels}):`;
    document.getElementById('lblStartPos').textContent = `${t.lblStartPos || 'Start Position'} (1-${maxLabels}):`;
    changeFormat(currentFormatKey);
}
...
function setupEventListeners() {
    ...
    document.getElementById('btn-update-locations').addEventListener('click', function() {
        ...
        const t = i18n[currentLang] || { locUrlInvalid: 'Invalid URL', locUrlUpdated: 'Updated' };
        const n2 = document.getElementById('n2');
        if (newUrl) {
            try {
                new URL(newUrl);
                ...
                n2.innerHTML = '✅ ' + (t.locUrlUpdated || 'Locations-URL updated!');
                n2.style.color = '#27ae60';
                loadLocations();
            } catch (e) {
                n2.innerHTML = '❌ ' + (t.locUrlInvalid || 'Invalid URL');
                n2.style.color = '#e74c3c';
            }
        } else {
            n2.innerHTML = '❌ ' + (t.locUrlInvalid || 'Invalid URL');
    ...
    document.getElementById('i6').addEventListener('blur', function() {
        ...
        localOverrides[artNr] = val;
        saveLocalOverrides();

        const item = a2.find(i => i.artNr === artNr);
        if (item) item.geb = val;

        const reportKey = artNr + '|' + val;
        const reported = JSON.parse(localStorage.getItem('lavu_reported_geb') || '{}');
        if (!reported[reportKey]) {
            fetch('https://sortiment-api.lavu-ooe.workers.dev/report', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    artNr: artNr,
                    geb: val,
                    bez: document.getElementById('i7').value.trim(),
                    clientId: clientId
                })
            }).catch(e => console.warn('Report failed', e));
            reported[reportKey] = true;
            localStorage.setItem('lavu_reported_geb', JSON.stringify(reported));
        }
    });
}
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (savedLocUrl) {
        ...
        document.getElementById('n2').innerHTML = '📍 ' + (i18n[currentLang]?.locUrlSaved || 'Locations-URL saved.');
